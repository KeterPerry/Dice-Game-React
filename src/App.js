...
substructOrSum=(total, current, index)=>{
  ...
  const currArr=[...this.state.currentScore]  
    ...
    console.log(index)
    ...
    else if(this.state.randomImgNum1===6 && this.state.randomImgNum2===6){
      currArr[index]=0;
     ...
     let newCurrent= (current) +this.state.randomImgNum1+this.state.randomImgNum2;
...
handleClickRollDice =()=>{   
  ...
  this.setState({isPlayer1:true,isPlayer2:false});
   this.substructOrSum(this.state.totalScore[0], this.state.currentScore[0], 0);
  }
   else{
    console.log("i'm false");
  this.setState({isPlayer1:false,isPlayer2:true});
  this.substructOrSum(this.state.totalScore[1], this.state.currentScore[1], 1);
  }
